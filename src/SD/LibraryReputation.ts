...
export type LibraryReputationMatch = {
  name: string;
  reason: string;
};

export type LibraryReputationResult = {
  reputation01: number;          // 0(최악) ~ 1(가장 양호)
  matches: LibraryReputationMatch[];
  reasons: string[];
};
 ...
 */
export function analyzeLibraryReputationFromCode(code: string): LibraryReputationResult {
  const reasons: string[] = [];
  const matches: LibraryReputationMatch[] = [];
