 ...
 */
function analyzeExternalCallAddition(code) {
    const reasons = [];
    // 네트워크/DB 등 외부 호출
    ...
    if (externalCalls > 0) {
        reasons.push(`Detected ${externalCalls} external network/DB calls (fetch/axios/http*/DB client).`);
    }
    if (ioCalls > 0) {
        reasons.push(`Detected ${ioCalls} file-system I/O calls (fs.*, readFileSync, writeFileSync, etc.).`);
    }
    return { externalCalls, ioCalls, reasons };
}
//# sourceMappingURL=ExternalCallAddition.js.map