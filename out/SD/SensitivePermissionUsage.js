...
exports.analyzeSensitivePermissionUsageFromCode = analyzeSensitivePermissionUsageFromCode;
const clamp01 = (x) => Math.max(0, Math.min(1, x));
function analyzeSensitivePermissionUsageFromCode(code) {
    const reasons = [];
    ...
    if (/\b(child_process|exec\(|spawn\(|system\(|popen\(|subprocess\.)/i.test(code)) {
        permRisk += 0.4;
        reasons.push("perm: OS command execution API (child_process/exec/spawn/system/popen/subprocess) detected");
    }
    // 2) 파일 시스템 접근 (fs.read/write/unlink/chmod/chown/readdir 등)
    if (/\bfs\.(read|write|unlink|chmod|chown|readdir)\b/i.test(code)) {
        permRisk += 0.3;
        reasons.push("perm: filesystem access via fs.* (read/write/unlink/chmod/chown/readdir) detected");
    }
    // 3) 시크릿/크리덴셜 접근 (process.env, secret, password, credential 등)
    if (/\bprocess\.env\b|secret|password|credential/i.test(lower)) {
        permRisk += 0.3;
        reasons.push("perm: access to process.env / secret / password / credential detected");
    }
...
//# sourceMappingURL=SensitivePermissionUsage.js.map