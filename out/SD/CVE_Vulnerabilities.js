...
exports.DYN_CVE_DB = exports.RULE_DB = void 0;
exports.loadGeneratedRuleDb = loadGeneratedRuleDb;
exports.loadGeneratedCveDb = loadGeneratedCveDb;
exports.vectorCveScan = vectorCveScan;
exports.regexHeuristicScoreFromDB = regexHeuristicScoreFromDB;
...
function loadGeneratedRuleDb(ctx) {
    try {
        ...
        exports.RULE_DB = arr;
        // 전역 토크나이저 룰도 보관
        exports.RULE_DB.tokenizer_rules = obj?.tokenizer_rules || [];
        return arr;
    }
    catch (e) {
...
function loadGeneratedCveDb(ctx) {
    try {
        ...
        exports.DYN_CVE_DB = Array.isArray(arr) ? arr : [];
        return exports.DYN_CVE_DB;
    }
    catch (e) {
...
function getSigDB() {
    return Array.isArray(exports.DYN_CVE_DB) ? exports.DYN_CVE_DB : [];
}
/** 토큰화에 사용할 정규식 패턴 수집 */
function collectTokenizerPatterns() {
    const globalRules = [];
    const rootRules = exports.RULE_DB?.tokenizer_rules;
    if (Array.isArray(rootRules))
        globalRules.push(...rootRules);
    for (const sig of exports.RULE_DB || []) {
        const arr = sig.tokenizer_rules;
        if (Array.isArray(arr))
            globalRules.push(...arr);
    }
    const perSigRegex = [];
    for (const sig of exports.DYN_CVE_DB || []) {
...
//# sourceMappingURL=CVE_Vulnerabilities.js.map